...
var stateID int
var stateRecorded, channel1To0, channel2To0 bool
...
func sendMarker() {
	...
	setUp1, err := net.ResolveUDPAddr("udp4", "127.0.0.1:49205")
	...
	_, err = connection1.Write([]byte("0"))
	...
	_, err = connection2.Write([]byte("0"))
...
func receiveMarker() {
	...
	for {
		if channel1To0 == true && channel2To0 == true {
			channel1To0 = false
			channel2To0 = false
			stateRecorded = false
		}
		...
		if gotRequestFrom == "1" && channel1To0 == false {
			//set channel state if necessary
			channel1To0 = true
		} else if gotRequestFrom == "2" && channel2To0 == false {
			//set channel state if necessary
			channel2To0 = true
		}
		if channel1To0 == true && channel2To0 == true {
...
func sendRequest(IP string) {
	...
	if reply == "SUCCESS" {
		ledgerKeeping[localTime.VectorTime] = NewLedger("0", cName, requestSend.StockCount, requestSend.StockPrice)
...
func initial() {
	...
	pId := "0" //sending the processID to the server
	pIdSend := []byte(pId + "\n")
...
func main() {
	...
	for {
		...
		if choice == 1 {
			...
			fmt.Println("Enter the system you want to connect with (1 or 2): ")
			fmt.Scanln(&system)
			if system == 1 {
