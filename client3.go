...
var stateID int
var stateRecorded, channel0To2, channel1To2 bool
...
func sendMarker() {
	...
	_, err = connection1.Write([]byte("2"))

	setUp2, err := net.ResolveUDPAddr("udp4", "127.0.0.1:49205")
	...
	_, err = connection2.Write([]byte("2"))

}

func receiveMarker() {

	s, err := net.ResolveUDPAddr("udp4", "127.0.0.1:49206")
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	for {
		if channel0To2 == true && channel1To2 == true {
			channel0To2 = false
			channel1To2 = false
			stateRecorded = false
		}
		...
		if gotRequestFrom == "0" && channel0To2 == false {
			//set channel state if necessary
			channel0To2 = true
		} else if gotRequestFrom == "1" && channel1To2 == false {
			//set channel state if necessary
			channel1To2 = true
		}
...
func sendRequest(IP string) {
	...
	if reply == "SUCCESS" {
		ledgerKeeping[localTime.VectorTime] = NewLedger("2", cName, requestSend.StockCount, requestSend.StockPrice)
...
func initial() {
	...
	pId := "2" //sending the processID to the server
	pIdSend := []byte(pId + "\n")
...
func connectServer(ledgerKeeping map[[3]int]*ledger) {
	...
	if strings.TrimSpace(string(dataGot)) == "SUCCESS" {
		...
		ledgerKeeping[localTime.VectorTime] = NewLedger("0", cName, int64(order), price)
...
func main() {
	...
	for {
		...
		if choice == 1 {
			...
			fmt.Println("Enter the system you want to connect with (0 or 1): ")
			fmt.Scanln(&system)
			if system == 0 {
				...
				syncTimeSender(Device1IP)
				sendRequest(Device1IP)
				sendMarker()
				time.Sleep(5 * time.Second)
			}
		} else if choice == 4 {
